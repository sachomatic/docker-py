...
class User:
    ...
    def is_admin(self):
        ...
        """
        if self.permissions[-1].value == True:
            return True
    ...
    def has_perm(self,perm:int):
        ...
        """
        if self.permissions[perm].value == True or self.is_admin():
            return True
...
BASE_PERMS = (Permission("See logs",True), # Elles sont par ordre de priorité
...
with db.db:
    def find_admin():
        ...
        for user in users:
            ...
            if permissions[-1]["value"] == True: # Si la dernière permission (admin) est True
    ...
    def pass_admin(user):
        ...
        st.session_state["user"].permissions[-1].value = True # On change les variables de session
        st.rerun()
